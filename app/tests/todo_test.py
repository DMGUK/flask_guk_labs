...
def test_add_todo(client, init_database, log_in_default_user):
    data = {
        'title': 'New Task',
        'description': 'Description of the new task'
    }
    response = client.post(url_for('todo.add_todo'), data=data, follow_redirects=True)
    assert response.status_code == 200
    assert b'Todo task has been successfully added' in response.data

def test_update_todo(client, init_database, log_in_default_user, todo):
    response = client.get(url_for('todo.update_todo', todo_id=1), follow_redirects=True)
